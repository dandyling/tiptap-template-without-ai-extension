...
export const TableOfContentPanel = memo(({ editor, onItemClick }: TableOfContentPanelProps) => {
  ...
  return (
    <>
      <div className="mb-2 text-xs font-semibold text-gray-500 uppercase">Table of contents</div>
        ...
        <div className="flex flex-col gap-1">
          {data.content.map(item => (
            <a
              ...
              onClick={onItemClick}
              className={cn(
                'block font-medium text-gray-500 p-1 rounded bg-opacity-10 text-sm hover:text-gray-800 transition-all hover:bg-black hover:bg-opacity-5 truncate w-full',
                item.isActive && 'text-gray-800 bg-gray-100',
              )}
            >
